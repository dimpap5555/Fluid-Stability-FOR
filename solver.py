...
class LinearNavierStokesSolver:
    ...
    def __init__(self,
        ...
        self.t_vals = np.linspace(0, t_total, int(t_total * fps))

        self.lambda_ = omega_s - alpha_s * u_inf
        self.resonance = np.isclose(self.lambda_, 0, atol=1e-10)
    ...
    def _compute_coefficients(self):
        if self.resonance:
            ...
            beta = np.sqrt(self.alpha_s ** 2 + self.lambda_ / (1j * self.nu))
            self.beta = beta
            beta_r, beta_i = np.real(beta), np.imag(beta)
            denom = (beta_r - self.alpha_s) ** 2 + beta_i ** 2
            a4 = self.A * self.omega_s * self.alpha_s * (beta_r - self.alpha_s) / denom
            b4 = -self.A * self.omega_s * self.alpha_s * beta_i / denom
            self.C4 = a4 + 1j * b4
    ...
    def v_hat(self, y):
        if self.resonance:
            return -self.A * self.omega_s * np.exp(-self.alpha_s * y)
    ...
    def dv_hat(self, y):
        if self.resonance:
            return self.A * self.omega_s * self.alpha_s * np.exp(-self.alpha_s * y)
    ...
    def d2v_hat(self, y):
        if self.resonance:
            return -self.A * self.omega_s * self.alpha_s ** 2 * np.exp(-self.alpha_s * y)
    ...
    def d3v_hat(self, y):
        if self.resonance:
            return self.A * self.omega_s * self.alpha_s ** 3 * np.exp(-self.alpha_s * y)
    ...
    def get_wall_forces(self, t):
        ...
        shear_hat = self.nu * (dUdy0 + 1j * self.alpha_s * V0)
        fx = np.real(shear_hat * np.exp(1j * (self.alpha_s * self.x - self.omega_s * t)))

        pressure_hat = -(self.rho / (1j * self.alpha_s ** 2)) * (
            self.lambda_ * dVdy0 + self.nu * (d3Vdy3 - self.alpha_s ** 2 * dVdy0)
        )
    ...
    def animate_velocity(self):
        ...
        ani = animation.FuncAnimation(fig, update, frames=len(self.t_vals), interval=1000 / self.fps)
        plt.tight_layout()
        plt.show()
        return ani
    ...
    def animate_wall_forces(self):
        ...
        ani = animation.FuncAnimation(fig, update_force, frames=len(self.t_vals), blit=True, interval=1000 / self.fps)
        plt.tight_layout()
        plt.show()
        return ani